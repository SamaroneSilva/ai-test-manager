    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-3xl md:text-4xl font-bold flex items-center gap-3">📋 Testes cadastrados</h2>
          <button
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {tests.map((t) => (
              <div key={t._id} className="bg-white rounded-2xl p-5 shadow border border-gray-100 hover:shadow-lg transition">
                <h3 className="text-lg font-semibold mb-1">{t.title}</h3>
                <p className="text-gray-600 mb-3">{t.description}</p>
                <p className="text-sm text-gray-400">Criado em: {new Date(t.createdAt).toLocaleString()}</p>
              </div>
            ))}
          </div>
        )}
